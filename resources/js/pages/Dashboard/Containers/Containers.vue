...
const {
  ...
  status,
  loadData,
} = requestOrderTable({ url: 'containers/getAll' })
    ...
    <div class="col-span-12 flex flex-col md:flex-row items-center justify-between gap-4 p-4">
      <div class="flex flex-col gap-2">
        ...
        <p class="text-sm dark:text-gray-400 text-gray-500">
          Administra los contenedores de reciclaje distribuidos por la ciudad.
        </p>
      </div>
    ...
    >
      <template #item.name="{ item }">
        <div class="flex flex-col">
          ...
          <div class="flex flex-row items-center justify-start gap-2 text-nowrap truncate">
            <span class="text-gray-400">ID: {{ item.id }}, Serial: {{ item.serial_number }}</span>
          </div>
        </div>
      </template>

      <template #item.location="{ item }">
        <div class="truncate max-w-xs flex flex-row items-center justify-start gap-2">
          <VIcon
            icon="bx-map-pin"
            size="small"
            :color="darkModeStore.darkMode ? 'white' : 'primary'"

...
const { loading, updateReward, setNewData, isUnchanged, rewardData } = useUpdateReward()
...
watch(() => props.modelValue, val => {
  ...
  if (val) {
    ...
    loadCatShops()
    isUnlimitedStock.value = props.data.stock === null
    isIndefiniteExpiration.value = props.data.expires_at === null
  }
  resetValidations()
})
...
const handleSaveReward = async () => {
  ...
  const result = await updateReward({
    isUnlimitedStock: isUnlimitedStock.value,
    isIndefiniteExpiration: isIndefiniteExpiration.value,
  })
